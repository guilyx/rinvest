...
class InvestmentStrategy(Strategy):
    def __init__(self, asset: str, initial_investment: float, start_date: Union[str, datetime], end_date: Union[str, datetime], recurrent_investment: Optional[RecurrentInvestment] = None):
        super().__init__(asset, initial_investment, start_date, end_date, recurrent_investment)
        self.asset = asset

    def calculate_return(self) -> StrategyPerformance:
        data = Ticker(self.asset).history(start=self.start_date, end=self.end_date)["Close"]
        ...
        if self.recurrent_investment:
            ...
            recurrent_investment_dates = pd.date_range(start=data.index[0], end=data.index[-1], freq=recurrent_investment_frequency)
        ...
        pnl = (daily_portfolio_values - daily_portfolio_values.shift(1)) - recurrent_investments
        total_return = daily_portfolio_values[-1] - self.initial_investment - total_recurrent_investment
        return StrategyPerformance(self.name, total_return, daily_portfolio_values, pnl, recurrent_investments)


class SavingsStrategy(Strategy):
    def __init__(self, name: str, annual_interest_rate: float, initial_investment: float, start_date: Union[str, datetime], end_date: Union[str, datetime], recurrent_investment: Optional[RecurrentInvestment] = None):
        super().__init__(name, initial_investment, start_date, end_date, recurrent_investment)
    ...
    def calculate_return(self) -> StrategyPerformance:
        ...
        daily_portfolio_values = pd.Series(self.initial_investment * (1 + daily_interest_rate) ** np.arange(len(index)), index=index)
        total_recurrent_investment = 0
        recurrent_investments = pd.Series(0, index=daily_portfolio_values.index)
        ...
        if self.recurrent_investment:
            recurrent_initial_investment = self.recurrent_investment.amount
            recurrent_investment_frequency = self.recurrent_investment.frequency
            recurrent_investment_dates = pd.date_range(start=index[0], end=index[-1], freq=recurrent_investment_frequency)

            for date in recurrent_investment_dates:
                recurrent_investments.loc[date] = recurrent_initial_investment

            total_recurrent_investment = recurrent_investments.sum()
            daily_portfolio_values += recurrent_investments.cumsum() * (1 + daily_interest_rate) ** np.arange(len(recurrent_investments))
...
class BackTester:
    ...
    def plot_portfolio_values(self, performances: Dict[str, StrategyPerformance], frequencies: List[str]):
        ...
        for i, ax in enumerate(axs.flatten()):
            for name, performance in performances.items():
                data = performance.daily_portfolio_values.resample(frequencies[i]).last()
                ax.plot(data, label=name)

            total_daily_values = pd.concat([p.daily_portfolio_values for p in performances.values()], axis=1).fillna(method='ffill').sum(axis=1)
            total_daily_values = total_daily_values.resample(frequencies[i]).last()
            ax.plot(total_daily_values, label='Cumulative', color='black', linestyle='--')
    ...
    def plot_performance(self, performances: Dict[str, StrategyPerformance], frequency='D'):
        ...
        for name, performance in performances.items():
            ...
            performance_pct_change = (performance.pnl / total_investments).resample(frequency).sum()
...
def convert_to_json(strategies: List[Union[InvestmentStrategy, SavingsStrategy]], filename: str):
    ...
    for strategy in strategies:
        ...
        if strategy.recurrent_investment is not None:
            strategy_dict["recurrent_investment"] = strategy.recurrent_investment.__dict__
